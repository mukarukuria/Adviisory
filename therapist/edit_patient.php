				...
				<table>
					...
					<tbody>
						...
						if(isset($_SESSION['user-id'])){
							$id = $_SESSION['user-id'];
							$sql = mysqli_query($conn, "SELECT * FROM session where therapist = $id");

							while($row = mysqli_fetch_assoc($sql)){
								$_SESSION['attended-id'] = $row['sessions_id'];
								?>
								<tr>
							...
							<?php
								...
								if($_SESSION['attended-id'] == $row['sessions_id']){
									$update_query = mysqli_query($conn, "UPDATE book_session SET session_status = 'attended' WHERE therapist = '$id'");	
								}
							}
						}
